...
| **🆕 通知的「实时」现实边界** | Actions cron 最小 5 分钟且常延迟 5–30 分钟 | 明确为**准实时**（采集触发后 ≤1 分钟发出；源发布到采集的轮询等待最坏另计 60 分钟，端到端典型 ≈50 分钟、最坏 ≈96 分钟），不承诺真实时；通知在 Actions 侧发送，见 §12 |
...
|---|---|---|---|---|---|---|
| A 同分支内联 | 构建期打进产物 | 需重建 | ≤60 分钟滞后 | — | — | **兜底/SEO 用** |
...
| **G13** 🆕 | 「实时」通知的真实边界？ | **准实时：采集后 ≤1 分钟发出**（端到端典型 ≈50 分钟、最坏 ≈96 分钟） | Actions cron 最小 5 分钟且常延迟（§12.1） |
...
> **「实时结果总结」= 准实时**：**采集完成之后 ≤ 1 分钟发出**；**端到端**（源发布 → 收到提醒）**典型 ≈ 50 分钟，最坏 ≈ 96 分钟（偶发 120+）**。
