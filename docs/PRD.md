...
| 数据调度 | GitHub Actions（每小时抓取；当天数据汇总为**一个文件**，次日转**历史数据**，按年/月目录保留**一年**，年初归档到 **GitHub Release**） |
...
> 3. 项目基于 GitHub 仓库，支持 GitHub Pages 与 Cloudflare Pages 部署，GitHub Action 每小时获取最新数据，每天数据沉淀为 JSON 或 CSV，去重，实时热点分析基于当天数据，只做当天分析。
...
> 2. 每小时获取一次最新数据，当天的所有数据都追加到一个 json 数据文件中，第二天的时候把前一天的 json 数据文件转成历史数据，历史数据按照保存一年来处理（分好年-月目录存放），并且增加一个每年把上一年的所有数据归档到 github release 中去，这样部署分支上最多只有一年的数据；
...
| 时间成本高 | 手动刷各站，容易漏 | Actions 每小时自动抓取，倒序展示最新 |
...
- ❌ 不做 WebSub 实时订阅推送（仍采用 60 分钟轮询）—— **【v1.1 澄清】** 此为「**源端实时推送协议**」；与 v1.1 新增的「**通知推送**」（F-100~F-107，把结果主动推给人）**不是同一件事**，通知能力**纳入范围**
- ❌ 不做服务端数据库 / 动态后端
...
| **热榜流** | Hot Stream | 页面1「聚合热榜」的全渠道聚合时间流。 |
...
| **当天数据文件** | Daily File | **【v1.1 新增】**某个自然日内、每小时采集结果**汇总到的同一个文件**（当天持续合并更新）。次日冻结为历史。 |
...
| US-10 | 作为**数据贡献者**，我希望系统自动每小时抓取、每天沉淀一份去重后的数据文件，以便站点数据始终新鲜且可追溯。 |
...
│
├── 6. 调度与部署层 (Ops)
│   ├── 6.1 GitHub Actions 采集调度 (cron 7 * * * *)
...
| 编号 | 功能名 | 所属模块 | 优先级 | 描述 | 验收要点 |
|---|---|---|---|---|---|
| F-030 | 每小时定时采集 | 6.1 | P0 | GitHub Actions cron `7 * * * *` | 定时触发成功 |
...
| 编号 | 功能名 | 所属模块 | 优先级 | 描述 | 验收要点 |
|---|---|---|---|---|---|
| F-040 | 页面1 聚合热榜 | 4.1 | P0 | 全渠道 Item 时间倒序列表 | 字段齐全、可跳转 |
...
| 编号 | 功能名 | 所属模块 | 优先级 | 描述 | 验收要点 |
|---|---|---|---|---|---|
| F-085 | 当天数据汇总单文件 | 3.2 | P0 | 当天每小时采集结果**合并进同一个当天数据文件** | 一个自然日**只有一个当天文件**；重复抓取只更新不新增 |
...
> **【v1.1 导航变化】** 主导航由 3 项扩展为 **4 项**：页面1 聚合热榜 / 页面2 渠道分栏看板 / 页面3 分析报告（当天）/ **页面4 历史趋势与回看**。页面3 保持「**只做当天**」的定位不变，历史能力**独立成页**（判断与理由见 §6.4）。
...
- 顶部区
  ...
  - 快捷入口：跳页面2 / 页面3
- 控制区
  - 排序：整合倒序——优先 `updatedAt`、缺失回落 `publishedAt`，整体倒序（无切换 UI）
  - 筛选：渠道筛选（多选）、分类筛选、时间范围（今天 / 全部，默认今天）
  - 搜索框：按标题/摘要关键词
  - **位置**：以上四项构成**一条横贯顶部的筛选条**（≥1024px 单列置顶、黏附页头之下），
    紧随其下是「共 N 条符合当前条件」计数行（主理人 2026-09-15 拍板：取消原 248px 左侧筛选栏——
    FilterBar 本身是横向 flex-wrap 条，塞进窄栏会被迫竖排堆叠并留出大片空白）
  - 视图密度（可选）
- 列表区（主内容）
  - 每条 Item 一行/一卡，倒序排列
  - 分页或懒加载
...
| 断点 | 行为 |
|---|---|
| 桌面（≥1024px） | **筛选条横贯顶部**（渠道 / 分类 / 时间 / 搜索一行）+ 全宽列表；字段全展示 |
| 平板（768–1023px） | 筛选默认折叠，「筛选」按钮展开（开关在面板上方）；列表为主 |
...
| 指标 | 目标 | 说明 |
|---|---|---|
| 实时通知发出延迟 | 采集完成起 **≤ 1 分钟** | 端到端（源发布 → 收到提醒）受 60 分钟轮询约束，最坏约 **60~90 分钟**（可接受，已与外话澄清） |
...
2. Actions 每小时自动采集，**当天数据汇总为一个文件**，次日转为历史（按年/月目录）（F-085~F-087）。
...
| 约束 | 数值 |
|---|---|
| Actions 最小 cron 间隔 | 5 分钟（我们用 60 分钟） |
...
| 7 | **`Q13` 就地更新为「已定」**：CF Pages 改直传、不再有生产分支监听问题；原「720/月超 500」顾虑消失 | 修改/已定 | Q13 |
