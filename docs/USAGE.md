...
> 线上运行时数据由**独立公开数据仓库**（`Shonee/rss-radar-data` 的 `deploy` 分支）提供，前端经构建期环境变量 `VITE_DATA_OWNER` / `VITE_DATA_REPO` 指向它；需先在 GitHub / Cloudflare 后台完成数据通道配置（详见 `DEPLOYMENT.md` §3.1.5），否则站点只读到 P1 fixture、首屏仅 3 条。

> ⚠️ 现状（详见 `DEPLOYMENT.md` §0）：`.github/workflows/` 下 6 个文件**已创建**，`collect` 已按每小时周期真实运行并维护 `deploy` 数据分支；**Cloudflare Pages 项目已创建（路径 A：Git 集成 —— 依据 2026-09-15 构建日志推断，未能本地验证）并触发过首次构建**，但该次构建**失败**于跨平台可选依赖问题（`@rollup/rollup-linux-x64-gnu`，已修复 lock，见 `DEPLOYMENT.md` §3.1.7），**部署端到端仍未跑通**；两个 deploy workflow 的 `push:` 自动触发段仍是注释。
