...
| **写模式** | **高频覆盖写**：每 30 分钟「读旧 → 按 `id` 合并 → 覆盖写」（`mergeWrite`） | **追加写**：每天新增一份，写后基本不改 |
...
- **运行时 fetch 的当天快照 → 紧凑单行**：省 30% 裸体积、解析略快；反正它每 30 分钟被覆盖，**没有 diff 价值**。
...
> **判断**：**CSV 不适合承担「读-合并-写」主链路**——它的嵌套往返有损，而 `mergeWrite` 又恰好每 30 分钟跑一次。**这一条与 §2.1 一起，双重锁定「当天主格式 = JSON」。**
...
2. **读-合并-写**：`mergeWrite` 每 30 分钟跑一次，JSON 原生对象改属性；CSV 嵌套往返有损。
...
| 数据分支策略 | **微调** | 从「纯 force-push」→「每 30 分钟 amend（当日内）+ 每日一次提交」，以保留跨天 provenance（见 §5.3） |
...
```
· 一天内的 48 次采集：git commit --amend + git push --force-with-lease
  → 当天只有 1 个「可变」提交，无 48 次噪音（保留原设计的「无膨胀」优点）
...
| C 每次采集都提交 | 48/天 | ✅ 有 | ❌ 高（≈48 commit/天） | 不用 |

> **结论**：**「force-push 会丢历史数据」是个误解——它丢的是 commit 历史，不是数据文件。** 但原设计确实**放弃了跨天 provenance**；用「**当日内 amend + 跨天新建提交**」即可**以接近零的成本**补回，且仍避免 48 次/天的 commit 噪音。**推荐改用策略 B。**
...
| **D2** | force-push **是否改为保留历史**？ | **改为「当日内 amend + 跨天新建提交」（策略 B）** | 近乎零成本拿回跨天 provenance，且仍无 48 次/天噪音（§5.3） |
