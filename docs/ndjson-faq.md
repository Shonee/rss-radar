...
| 第 12 轮（约 6h） | 120 条 | 读 100KB + 写 100KB | 追加 1 行 |
| 第 24 轮 | 240 条 | 读 200KB + 写 200KB | 追加 1 行 |
| 第 48 轮 | 250 条 | 读 209KB + 写 209KB | 追加 1 行 |
| **全天累计 I/O** | — | **≈ 5 MB 读 + 5 MB 写（且逐轮变慢）** | **≈ 12KB（恒定，一行）** |

> 一天下来，**NDJSON 省掉了约 99.8% 的磁盘 I/O**，并且每一轮的速度恒定（不会越跑越慢）。
