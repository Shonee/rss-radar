...
| ② | Actions 每 30 分钟采集 + 次日转历史 | ⛔ 未验证（需远程仓库 + Actions 权限/Secrets） |
| ③ | 四页面 + 趋势图 | ✅ 已满足 |
...
- **当前证据**：
  ...
  - **代码侧缺口**：`scripts/collect/main.mjs` 的 `collect` 主流程 **没有调用** `rolloverIfNewDay`（仅在 `scripts/dev/seed-history.mjs` 与单测/e2e 中调用）。即「次日转历史」能力就绪，但尚未串联进生产采集链路 —— 需在 `collect.yml` 于 collect 之后追加 rollover 步骤（或确认 workflow 已串联）。
  - **调度侧 (a)**：`.github/workflows/collect.yml` 已存在，调度 `cron: '7,37 * * * *'`（约每 30 分钟）。但 `.github/` 属另一位 worker 领地，**本 worker 未改动**；其真实触发需推送至 GitHub 远程仓库。
- **状态**：⛔ 未验证（真实触发）。
  - **需老登提供**：① 把仓库 push 到 GitHub 远程仓库；② 配置 Actions 运行权限与所需 Secrets（如源鉴权）；③ 确认 `collect.yml` 在 collect 后串联 `rolloverIfNewDay`（建议补一步），使「次日转历史」真正随调度自动发生。
  - 本地可由 `node tests/e2e/full-pipeline.mjs` 证明「采集→归档→读回」逻辑正确，但无法代替远程 Actions 真实调度验证。
...
| 项 | 依赖 |
|---|---|
| ② Actions 真实触发 + 次日转历史 | 远程 GitHub 仓库 + Actions 权限/Secrets；确认 `collect.yml` 串联 `rolloverIfNewDay` |
