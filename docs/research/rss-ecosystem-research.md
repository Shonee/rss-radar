...
>
> ⚠️ **后续变更（2026-09-15）**：本文成稿时按「每 30 分钟采集」评估可行性。项目**最终采用每小时**（`.github/workflows/collect.yml` 的 `cron: '7 * * * *'`，见 `docs/PRD.md` v1.1）。下文出现的「每 30 分钟」均为**当时调研口径**，不代表当前实现；其中**平台事实**（Actions 最小间隔 5 分钟、`*/30 * * * *` 语法合法等）与频率决策无关，不受影响。
