...
describe('exclude / 关键词命中', () => {
  it('① 关键词命中：标题含「招聘」被排除', () => {
    ...
    assert.strictEqual(r.hitRuleId, 'r1');
    assert.match(r.hitField, /title/);
  });
  ...
  it('①b 关键词大小写无关（默认 caseSensitive=false）', () => {
    ...
    assert.strictEqual(evaluateItem(mkItem({ title: 'we are HIRING' }), rules).excluded, true);
  });
  ...
  it('①c 关键词 caseSensitive=true 时不命中', () => {
    ...
    assert.strictEqual(evaluateItem(mkItem({ title: 'we are hiring' }), rules).excluded, false);
  });
});
...
describe('exclude / 域名通配符', () => {
  it('② 域名 contains：url host 含 ad.example 被排除', () => {
    ...
    assert.strictEqual(evaluateItem(mkItem({ url: 'https://tracker.ad.example/click' }), rules).excluded, true);
  });
  ...
  it('②b 域名 equals：精确匹配被排除', () => {
    ...
    assert.strictEqual(evaluateItem(mkItem({ url: 'https://spam.com/post' }), rules).excluded, true);
    assert.strictEqual(evaluateItem(mkItem({ url: 'https://www.spam.com/post' }), rules).excluded, false);
  });
});
...
describe('exclude / 标题正则', () => {
  it('③ 标题正则：用 re2 兼容语法命中标题', () => {
    ...
    assert.strictEqual(evaluateItem(mkItem({ title: 'Show HN: a thing' }), rules).excluded, true);
    assert.strictEqual(evaluateItem(mkItem({ title: 'Ask HN: how to ...' }), rules).excluded, true);
    assert.strictEqual(evaluateItem(mkItem({ title: 'Normal article' }), rules).excluded, false);
  });
});
...
describe('exclude / 分类排除', () => {
  it('④ 分类级：category=finance 被排除', () => {
    ...
    assert.strictEqual(evaluateItem(mkItem({ category: ['tech_blog', 'finance'] }), rules).excluded, true);
    assert.strictEqual(evaluateItem(mkItem({ category: ['tech_blog'] }), rules).excluded, false);
  });
});
...
describe('exclude / 整源停用', () => {
  it('⑤ blockedChannels 返回被 channel: scope 停用的 id 集合', () => {
    ...
    assert.strictEqual(set.has('v2ex'), true);
    assert.strictEqual(set.has('spam'), true);
    assert.strictEqual(set.has('disabled'), false);
  });
  ...
  it('⑤b global channel scope：value 数组里的 id 全部停用', () => {
    ...
    const set = blockedChannels(rules);
    assert.strictEqual(set.has('v2ex'), true);
    assert.strictEqual(set.has('hn'), true);
  });
});
...
describe('exclude / hitCount 回填语义', () => {
  it('⑥ explain() 返回被哪条规则命中哪个字段', () => {
    ...
    assert.strictEqual(res.hitRuleId, 'r-hiring');
  });
});
...
describe('exclude / scope 维度', () => {
  it('scope=source:X 仅对指定 sourceId 生效', () => {
    ...
    assert.strictEqual(evaluateItem(mkItem({ sourceId: 'special', title: 'forbidden word' }), rules).excluded, true);
    assert.strictEqual(evaluateItem(mkItem({ sourceId: 'other', title: 'forbidden word' }), rules).excluded, false);
  });
});
